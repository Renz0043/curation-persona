...
| MCP Server | キュレーション結果を外部AIツールに公開（6ツール） | Should |
...
| ブックマーク登録 | Safari ショートカット経由の記事登録 + 自動深掘り | Should |
| Gemini Embedding | 記事タイトルのベクトル化 + 類似記事検索 | Should |
...
| **AI/LLM** | Gemini 2.5 Pro / Flash (Vertex AI), Gemini Embedding (text-embedding-004) |
| **Database** | Cloud Firestore |
| **Integration** | RSS Feeds, Safari Shortcuts (Apple Shortcuts) |
...
- [x] ディレクトリ構造作成
  ```
  ├── apps/web/          # Next.js
  ├── services/agents/   # Python (Collector/Librarian/Researcher + shared)
  └── docs/              # 設計ドキュメント
  ```
...
#### Step 2-4: Gemini Embedding 統合
- [x] 記事タイトル + meta_description のベクトル化（text-embedding-004）
- [x] Firestore への title_embedding 保存
- [x] ベクトル類似記事検索（Firestore Vector Search / COSINE距離）
...
#### Step 3-5: Safari ブックマーク自動深掘り
- [x] API Key 認証（Firebase Auth が使えない環境向け）
- [x] Safari ショートカット / Apple Shortcuts 経由の記事登録
- [x] WebScraper によるコンテンツ取得 + 自動深掘りレポート生成
- [x] ブックマーク専用コレクション（`bm_{userId}`）

#### Step 3-6: 異業種フィードバック
- [x] 深掘りレポート完成後、記事課題の抽象化
- [x] 12業種リストからランダム2業種の専門家視点でフィードバック生成
- [x] `cross_industry_feedback` フィールドとして保存

#### Step 3-7: MCPサーバー
- [x] FastMCP (stdio transport) によるキュレーション結果の外部公開
- [x] 6ツール実装（今日のブリーフィング、日付指定、記事詳細、興味プロファイル、高評価記事、類似記事検索）
- [x] Claude Desktop からの `.mcp.json` 接続対応
