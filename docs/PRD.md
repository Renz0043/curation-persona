...
| 機能 | 説明 | 優先度 |
|------|------|--------|
| Hunter Agent | RSS/Webニュースの巡回、Gemini 2.5 Flashによる初期スクリーニング | Must |
...
| Task Queue | Cloud Pub/Subによる非同期エージェント連携 | Must |
...
| 処理 | モデル | 理由 |
|------|--------|------|
| スクリーニング（大量） | Gemini 2.5 Flash | 低コスト・高速 |
| 詳細分析（少量） | Gemini 2.5 Pro | 高精度・広コンテキスト |
...
| **Infra/Deployment** | Firebase App Hosting, Cloud Scheduler, Cloud Pub/Sub |
...
| Agent間通信 | A2A / Pub/Sub | 標準プロトコル、疎結合 |
...
- [ ] ディレクトリ構造作成
  ```
  ├── apps/web/          # Next.js
  ├── services/agents/   # Python (Hunter/Librarian/Researcher)
...
- [ ] Cloud Run API 有効化
- [ ] Pub/Sub トピック作成（batch-trigger, research-request）

#### Step 1-3: 疎通確認
- [ ] Next.js ローカル起動確認
- [ ] Cloud Scheduler → Pub/Sub → Cloud Run 接続確認
- [ ] Firestore 読み書きテスト
...
#### Step 3-1: Hunter Agent
- [ ] RSS取得（feedparser）
- [ ] Gemini Flash による初期スクリーニング
...
#### Step 3-3: パイプライン統合
- [ ] Hunter → Librarian → Researcher 連携フロー
- [ ] A2A通信実装
