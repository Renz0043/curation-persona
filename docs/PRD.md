...
- [ ] 5段階評価 + コメントのデータモデル設計
- [ ] フィードバックAPI実装（rating + comment）
- [ ] Firestoreへの評価データ保存

#### Step 2-2: LLMベーススコアリング実装
- [ ] 過去の高評価記事（4-5★）をFirestoreから取得
- [ ] Gemini Flashで「ユーザー興味プロファイル」を生成
- [ ] 新記事をプロファイルに基づいてスコアリング

#### Step 2-3: Librarian Agent
- [ ] LLMベーススコアリングロジック実装
- [ ] コールドスタート対応（評価データ0件時の処理）
...
#### Step 3-3: パイプライン統合
- [ ] Collector → Librarian の連携フロー
- [ ] Dashboard API → Researcher の手動トリガーフロー
- [ ] A2A通信実装
